...
if ( ! class_exists( 'Astra_Custom_Nav_Walker' ) ) {
 ...
 // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
			...
			if ( '' != $this->megamenu && isset( $item->megamenu_content_src ) && 'default' != $item->megamenu_content_src ) {
				...
				switch ( $item->megamenu_content_src ) {
					...
					case 'custom_text':
						...
						// Defence in depth: set_mega_menu_option() already saves this meta through wp_kses_post().
						// Same filter as the save side deliberately, so the two cannot drift apart.
						$content .= do_shortcode( wp_kses_post( $item->megamenu_custom_text ) );
						$content .= '</div>';
		...
		public function end_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$menu_id = is_object( $args ) ? $args->menu_id : ( isset( $args['menu_id'] ) ? $args['menu_id'] : '' );

			// Mirrors the guard in start_lvl() that opens the .astra-full-megamenu-wrapper div, so the closing </div> is output if and only if the wrapper was opened.
			if (
				0 === $depth
				&& '' != $this->megamenu
				&& 'ast-hf-mobile-menu' !== $menu_id
				&& 'ast-desktop-toggle-menu' !== $menu_id
				&& ( 'full' === $this->megamenu_width || 'full-stretched' === $this->megamenu_width )
			) {
				$output .= "{$indent}</div>\n";
			}
		}
