 ...
 // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
		...
		public function sticky_replace_header_logo_attr( $attr, $attachment, $size ) {
			...
			if ( $custom_logo_id == $attachment->ID ) {
				...
				// Detect the extension from the URL path directly, as wp_check_filetype() returns false when SVG is not in the site's allowed mime types.
				$file_extension = strtolower( pathinfo( (string) wp_parse_url( $attr['src'], PHP_URL_PATH ), PATHINFO_EXTENSION ) );

				if ( 'svg' === $file_extension ) {
					$existing_classes = isset( $attr['class'] ) ? $attr['class'] : '';
					if ( false === strpos( $existing_classes, 'astra-logo-svg' ) ) {
