...
function astra_ext_sticky_header_with_site_layouts_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {
	...
	if ( 'ast-box-layout' === $site_layout ) {
		...
		// Center the fixed header without relying on .ast-sticky-active — the class is already removed while the slide/fade exit transition is still visible.
		$box_layout_sticky_css = array(
		...
		$parse_css .= astra_parse_css( $box_layout_sticky_css );
	}
