 ...
 ;(function ( $, window, undefined ) {
		...
		if (
			...
			if ( jQuery( window ).scrollTop() > stick_upto_scroll ) {
				...
				if ( '1' === self.options.hide_on_scroll ) {
					...
					var sticky_active_css = {
						'max-width'      : max_width,
						'padding-top'    : self.options.shrink.padding_top,
						'padding-bottom' : self.options.shrink.padding_bottom,
					};

					// Box layout: margin design options override the auto margins on .ast-sticky-active, pinning the capped bar to the left edge. Only above the box width — below it max-width never binds and the design margins must stay as the visible inset.
					if ( 'ast-box-layout' === self.options.site_layout && $( window ).width() > parseInt( max_width ) ) {
						sticky_active_css['margin-left']  = 'auto';
						sticky_active_css['margin-right'] = 'auto';
					}

					selector.addClass( 'ast-sticky-active' ).stop().css( sticky_active_css );
	...
	astExtSticky.prototype.update_attrs = function () {
		...
		if ( 'none' == self.options.header_style ) {
			...
			if ( 'ast-padded-layout' === self.options.site_layout || 'ast-box-layout' === self.options.site_layout ) {
