...
if ( ! class_exists( 'Astra_Ext_Advanced_Headers_Markup' ) ) {
	...
	class Astra_Ext_Advanced_Headers_Markup {
		...
		public function header_custom_menu_text( $value ) {
			...
			// Not sanitized: the theme's astra_get_custom_html() applies wp_kses_post() to this return value.
			// A kses pass here would precede do_shortcode() and mangle `&` in shortcode attributes.
		...
		public function archive_title( $title ) {
			...
			if ( is_category() ) {
				...
				$title = '<span ' . astra_attr( 'archive-meta-author', array( 'class' => '' ) ) . '>' . esc_html( get_the_author() ) . '</span>';
			} elseif ( is_tax() ) {
		...
		/**
		 * Restrict the Page Header layout to templates that ship with the addon, else 'disable'.
		 *
		 * @since 4.13.9
		 * @param  mixed $layout Raw layout value from post meta.
		 * @return string Known layout slug, or 'disable'.
		 */
		public static function get_sanitized_layout( $layout ) {

			$allowed_layouts = array(
				'advanced-headers-layout-1',
				'advanced-headers-layout-2',
			);

			if ( is_string( $layout ) && in_array( $layout, $allowed_layouts, true ) ) {
				return $layout;
			}
		...
		public function load_markup() {
			...
			// Get our options.
			$advanced_headers_layout         = self::get_sanitized_layout( Astra_Ext_Advanced_Headers_Loader::astra_advanced_headers_layout_option( 'layout' ) );
			$advanced_headers_merged         = Astra_Ext_Advanced_Headers_Loader::astra_advanced_headers_layout_option( 'merged' );
			$advanced_header_bg_size         = Astra_Ext_Advanced_Headers_Loader::astra_advanced_headers_design_option( 'bg-size' );
			$advanced_header_parallax_device = Astra_Ext_Advanced_Headers_Loader::astra_advanced_headers_design_option( 'parallax-device' );
