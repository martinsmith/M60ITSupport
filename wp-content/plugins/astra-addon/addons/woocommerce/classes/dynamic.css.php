...
function astra_woocommerce_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {
	...
	if ( ( is_shop() || is_product_taxonomy() ) ) {
		if ( in_array( 'filters', astra_get_option( 'shop-toolbar-structure', array() ) ) ) {
			...
			if ( 'shop-filter-flyout' === astra_get_option( 'shop-filter-position' ) && is_active_sidebar( 'astra-woo-product-off-canvas-sidebar' ) ) {
				// Covers the Shop page and all product taxonomy archives; the Astra theme keeps a version-gated copy of the Shop-page rules only for older addon versions.
				// The 601px breakpoint intentionally matches the WooCommerce Product Filters block's min-width: 601px desktop breakpoint, overlapping it by 1px to avoid a fractional-viewport gap where the double popup would persist.
				$woo_off_canvas_product_filters_block = '
					@media ( max-width: 601px ) {
						.woocommerce-page .astra-off-canvas-sidebar .wp-block-woocommerce-product-filters {
							width: 100%;
						}
						.woocommerce-page .astra-off-canvas-sidebar .wp-block-woocommerce-product-filters :is(.wc-block-product-filters__open-overlay, .wc-block-product-filters__overlay-header, .wc-block-product-filters__overlay-footer) {
							display: none;
						}
						.woocommerce-page .astra-off-canvas-sidebar .wp-block-woocommerce-product-filters .wc-block-product-filters__overlay {
							pointer-events: auto;
							position: relative;
							width: 100%;
						}
						.woocommerce-page .astra-off-canvas-sidebar .wp-block-woocommerce-product-filters .wc-block-product-filters__overlay .wc-block-product-filters__overlay-dialog {
							position: relative;
							transform: translateY(0);
						}
						.woocommerce-page .astra-off-canvas-sidebar .wp-block-woocommerce-product-filters .wc-block-product-filters__overlay-dialog .wc-block-product-filters__overlay-content {
							overflow: auto;
							padding: 2px;
						}
					}
				...
				$css_output .= Astra_Enqueue_Scripts::trim_css( $woo_off_canvas_product_filters_block );
			}
		}
