 ...
 // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
		...
		public function modern_my_account_template() {
			if ( true === astra_get_option( 'modern-woo-account-view', false ) && is_account_page() ) {
				// Memberships swaps in its own members area navigation there - checked per view, not per plugin.
				$has_vendor_navigation = function_exists( 'wc_memberships_is_members_area_section' ) && wc_memberships_is_members_area_section();

				/**
				 * Filters whether Astra's modern my-account navigation replaces WooCommerce's default.
				 *
				 * Defaults to false while another plugin renders its own navigation for the current
				 * view - return true to force Astra's navigation regardless.
				 *
				 * @param bool $use_modern_navigation Whether to render Astra's modern navigation.
				 * @since 4.13.8
				 */
				$use_modern_navigation = apply_filters( 'astra_addon_woo_account_navigation_override', ! $has_vendor_navigation );
		...
		public function shop_js_localize( $localize ) {
			...
			// Add edit post url for logged in users.
