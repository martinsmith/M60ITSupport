	...
	class Astra_Ext_Advanced_Hooks_Meta {
			...
			?>
				<div class="ast-advanced-hook-enable-php-wrapper">
					<button type="button" class="ast-advanced-hook-enable-php-btn button button-primary button-large" data-editor-type="<?php echo esc_attr( $editor_type ); ?>" data-label="<?php echo esc_attr( $label ); ?>" >
						<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="m17 6 6 6-6 6"/><path d="m7 6-6 6 6 6"/><path d="m14.5 3-5 18"/></svg>
		...
		public function php_editor_markup( $post ) {
			...
			if ( ASTRA_ADVANCED_HOOKS_POST_TYPE === $post_type ) {
				...
				$this->maybe_render_snippet_state_notice( $post->ID );
				?>
		...
		/**
		 * Explain the stored snippet's front-end state when it is not the obvious one.
		 *
		 * @since 4.13.8
		 * @param int $post_id Post Id.
		 * @return void
		 */
		private function maybe_render_snippet_state_notice( $post_id ) {

			// Code meta is retained when a layout switches to the WordPress editor, so this would
			// otherwise fire on layouts that have no code editor at all.
			...
			// Same predicate as the front end, so this cannot disagree with what actually runs.
			if ( astra_addon_is_snippet_executable() ) {
				...
				$detail = get_userdata( $author_id )
					? __( 'The user it is assigned to is no longer an administrator.', 'astra-addon' )
					: __( 'The user it was assigned to no longer exists.', 'astra-addon' );
				?>
				<div class="notice notice-info inline ast-advanced-hook-snippet-orphaned">
					<p>
						<strong><?php esc_html_e( 'This snippet is running.', 'astra-addon' ); ?></strong>
						<?php echo esc_html( $detail ); ?>
						<?php esc_html_e( 'Only administrators can create or edit these layouts, so this code is still trusted and runs. Review it below and clear it if it should no longer run.', 'astra-addon' ); ?>
					</p>
				</div>
				<?php
				return;
			...
			?>
			<div class="notice notice-warning inline ast-advanced-hook-snippet-inactive">
				<p>
					<strong><?php esc_html_e( 'This snippet is not running.', 'astra-addon' ); ?></strong>
					<?php esc_html_e( 'PHP execution is disabled on this site, so the code is not executed and is stripped from the front end.', 'astra-addon' ); ?>
				</p>
			</div>
			<?php
		}
