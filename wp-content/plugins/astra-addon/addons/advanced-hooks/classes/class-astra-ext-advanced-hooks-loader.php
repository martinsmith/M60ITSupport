	...
	class Astra_Ext_Advanced_Hooks_Loader {
		...
		public function advanced_hooks_post_type() {
			...
			$args = array(
				...
				// Admin-only: these layouts can hold an executable PHP snippet, so creating/editing
				// them requires manage_options across all vectors (admin UI, XML-RPC, REST).
		...
		public function load_react_script() {
			...
			// Spectra Blocks plugin slug.
			$plugin_slug = 'spectra-blocks/spectra-blocks.php';

			// Legacy Spectra keeps the notice hidden on sites already running it.
			$legacy_plugin_slug = 'ultimate-addons-for-gutenberg/ultimate-addons-for-gutenberg.php';
			...
			wp_localize_script(
				...
				array(
					...
					'isPluginActivated'          => is_plugin_active( $plugin_slug ) || is_plugin_active( $legacy_plugin_slug ),
