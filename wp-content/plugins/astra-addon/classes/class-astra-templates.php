...
if ( ! function_exists( 'astra_addon_get_template' ) ) {
	...
	function astra_addon_get_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
		...
		// Reject relative path segments before resolving; backstop for callers that build names from stored option/meta values.
		if ( ! is_string( $template_name ) || false !== strpos( $template_name, '..' ) ) {
			return;
		}
