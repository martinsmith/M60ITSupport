...
if ( ! function_exists( 'bsf_set_options' ) ) {
	...
	function bsf_set_options() {
		// Force check updates.
		if ( isset( $_GET['force-check-update'] ) || isset( $_GET['force-check'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Authorized by the capability check below.
			...
			$force_check = isset( $_GET['force-check'] ) ? sanitize_text_field( wp_unslash( $_GET['force-check'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Authorized by the capability check above.

			if ( 'update-core.php' === $pagenow && '1' === $force_check ) {
