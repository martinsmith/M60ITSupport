...
// Re-sync stored versions before WP core regenerates the update transients on
// `upgrader_process_complete` (core hooks `wp_update_plugins`/`wp_update_themes` at 10),
// so a just-installed version is not offered as an update again.
add_action( 'upgrader_process_complete', 'bsf_update_all_product_version', 5, 0 );

if ( ! function_exists( 'bsf_get_current_version' ) ) {
	/**
	 * Get current version of plugin / theme.
	 *
	 * Always reads from disk — deliberately not cached, because callers on
	 * `upgrader_process_complete` run after the files have changed mid-request.
	 *
