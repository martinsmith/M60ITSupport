...
if ( ! function_exists( 'bsf_product_updates_disabled' ) ) {
	/**
	 * Whether update checks are disabled for a product via its
	 * `BSF_{PRODUCT_ID}_CHECK_UPDATES` constant.
	 *
	 * The product id is uppercased with dashes converted to underscores, e.g.
	 * `astra-pro-sites` maps to the `BSF_ASTRA_PRO_SITES_CHECK_UPDATES` constant.
	 * Updates are treated as disabled when that constant is defined and set to
	 * boolean `false` or the string `'false'`.
	 *
	 * @since 1.29.18
	 *
	 * @param string $product_id Product ID.
	 * @return bool True if update checks are disabled for the product.
	 */
