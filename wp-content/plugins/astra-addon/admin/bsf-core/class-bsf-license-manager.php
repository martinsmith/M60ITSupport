...
if ( ! class_exists( 'BSF_License_Manager' ) ) {
	...
	class BSF_License_Manager {
		...
		public function get_remote_license_status( $purchase_key, $product_id ) {
			...
			$license_status = 'registered' === $this->bsf_get_product_info( $product_id, 'status' ) ? '1' : '0';
			$reply_readable = false;
			...
			if ( ! is_wp_error( $response ) && wp_remote_retrieve_response_code( $response ) === 200 ) {
				...
				// Only a body that is actually our payload is read - a WAF challenge, an error page or a truncated body keeps the status set above.
				if ( is_array( $response_body ) && isset( $response_body['status'] ) ) {
					...
					if ( true === $status || 'true' === $status || 1 === $status || '1' === $status ) {
						$license_status = '1';
					} elseif ( false === $status || 'false' === $status || 0 === $status || '0' === $status ) {
						$license_status = '0';
					}
			...
			// Save license status in transient - 6 hours for a real answer, 1 hour for an unreadable reply.
			set_transient( $transient_key, $license_status, $reply_readable ? 6 * HOUR_IN_SECONDS : HOUR_IN_SECONDS );
