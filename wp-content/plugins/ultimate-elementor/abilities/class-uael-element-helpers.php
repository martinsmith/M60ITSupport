...
class UAEL_Element_Helpers {
	...
	public static function parse_elementor_data( $post_id ) {
		...
		if ( ! is_array( $data ) ) {
			return new \WP_Error(
				'uael_invalid_elementor_data',
				__( 'Template has invalid Elementor data.', 'uael' ),
				array( 'status' => 500 )
			);
		}
	...
	public static function save_elementor_data( $post_id, $data ) {
		...
		if ( class_exists( '\Elementor\Plugin' ) ) {
			...
			if ( $document ) {
				...
				try {
					$result = $document->save( array( 'elements' => $clean_data ) );
				} catch ( \TypeError $e ) {
					$result = new \WP_Error( 'uael_document_save_error', $e->getMessage() );
				}
		...
		if ( false === $json ) {
			return new \WP_Error(
				'uael_encode_failed',
				__( 'Failed to encode Elementor data.', 'uael' ),
				array( 'status' => 500 )
			);
		}
	 ...
	 * @var string
	 */
	const UNDO_SNAPSHOT_META = '_uael_ai_undo_snapshot';
	 ...
	 *
	 * Runs only for users lacking `unfiltered_html`. Targets settings that can
	 * carry executable markup regardless of widget type — custom HTML attributes
	 * (event handlers, javascript: URLs), custom CSS classes (attribute breakout),
	 * and custom CSS blocks (</style> breakout) — at any nesting depth, e.g.
	 * link.custom_attributes inside a button or icon widget.
	 *
	 * @since 1.45.0
	 *
	...
	private static function sanitize_unsafe_setting_value( $key, $value ) {
		...
		if ( 'custom_attributes' === $key ) {
			...
			foreach ( (array) $lines as $line ) {
				...
				if ( 0 === strpos( $attr, 'on' ) || in_array( $attr, array( 'style', 'srcdoc', 'formaction', 'href', 'xlink:href' ), true ) ) {
					continue;
				}
		...
		// Custom CSS blocks (Pro) — CSS must never contain markup; strip angle
		// brackets to prevent a </style> breakout.
	...
	public static function insert_element( $elements, $new_element, $position = 'append' ) {
		...
		if ( 'append' === $position || 'prepend' === $position ) {
			...
			if ( null === $path ) {
				...
				$wrapper    = self::create_wrapper( array( $new_element ) );
				$elements[] = $wrapper;
				return $elements;
			}
		...
		if ( is_numeric( $position ) ) {
			...
			array_splice( $target, $index, 0, array( $new_element ) );
			return $elements;
		}
		...
		return new \WP_Error(
			'uael_invalid_position',
			__( 'Invalid position. Use "append", "prepend", or an object with "after", "before", or "inside" key.', 'uael' ),
			array( 'status' => 400 )
		);
	}
	...
	public static function remove_element( $elements, $element_id ) {
		...
		if ( ! $result['found'] ) {
			return new \WP_Error(
				...
				__( 'Element not found in template.', 'uael' ),
				array( 'status' => 404 )
			);
		}
	 ...
	 */
	public static function build_widget_element( $widget_type, $settings = array() ) {
		return array(
	 ...
	 */
	public static function create_wrapper( $children = array() ) {
	 ...
	 */
	public static function create_container( $children = array() ) {
		return array(
			'id'       => self::generate_element_id(),
			'elType'   => 'container',
			'isInner'  => false,
	 ...
	 */
	public static function create_section_column( $children = array() ) {
		return array(
			'id'       => self::generate_element_id(),
			'elType'   => 'section',
			'settings' => array(
	 ...
	 */
	public static function create_section( $column_sizes = array( 100 ), $is_inner = false, $settings = array() ) {
		$columns = array();
	 ...
	 */
	public static function create_layout( $column_sizes = array( 100 ), $is_inner = false, $settings = array() ) {
	...
	public static function build_section_from_definition( $definition ) {
		...
		$children        = isset( $definition['children'] ) && is_array( $definition['children'] ) ? $definition['children'] : array();
		$columns         = isset( $definition['columns'] ) && is_array( $definition['columns'] ) ? array_map( 'absint', $definition['columns'] ) : array( 100 );
		$column_settings = isset( $definition['column_settings'] ) && is_array( $definition['column_settings'] ) ? $definition['column_settings'] : array();
		...
		foreach ( $children as $child_def ) {
			...
			if ( 'widget' === $child_type ) {
				...
				$widget_settings = isset( $child_def['settings'] ) && is_array( $child_def['settings'] ) ? $child_def['settings'] : array();
				...
				if ( ! self::is_widget_allowed( $widget_type ) ) {
					...
					return new \WP_Error(
						...
						sprintf( __( 'Widget type "%s" is not recognized. Use template-builder/list-widget-types to see available types.', 'uael' ), $widget_type ),
						array( 'status' => 400 )
					);
				}
	...
	public static function add_column_to_section( $elements, $section_id, $column_size = 0, $auto_resize = true ) {
		...
		if ( ! $result['found'] ) {
			return new \WP_Error(
				'uael_section_not_found',
				/* translators: %s: element ID */
				sprintf( __( 'Section "%s" not found in template.', 'uael' ), sanitize_text_field( $section_id ) ),
				array( 'status' => 404 )
			);
		}
	...
	private static function find_widget_container( $elements ) {
		$container_types = array( 'section', 'column', 'container' );
		...
		foreach ( $elements as $index => $element ) {
			...
			if ( ! empty( $element['elements'] ) ) {
				...
				if ( in_array( $first_child_type, $container_types, true ) ) {
					...
					if ( null !== $deeper ) {
						return array_merge( array( $index ), $deeper );
					}
				}
			}
	...
	private static function insert_relative( $elements, $new_element, $sibling_id, $direction ) {
		...
		if ( ! $result['found'] ) {
			return new \WP_Error(
				'uael_sibling_not_found',
				/* translators: %s: element ID */
				sprintf( __( 'Sibling element "%s" not found in template.', 'uael' ), sanitize_text_field( $sibling_id ) ),
				array( 'status' => 404 )
			);
		}
	...
	private static function insert_relative_recursive( $elements, $new_element, $sibling_id, $direction ) {
		foreach ( $elements as $index => $element ) {
			if ( isset( $element['id'] ) && $element['id'] === $sibling_id ) {
				...
				array_splice( $elements, $offset, 0, array( $new_element ) );
	...
	private static function insert_inside( $elements, $new_element, $parent_id ) {
		...
		if ( ! $result['found'] ) {
			return new \WP_Error(
				'uael_parent_not_found',
				/* translators: %s: element ID */
				sprintf( __( 'Parent element "%s" not found in template.', 'uael' ), sanitize_text_field( $parent_id ) ),
				array( 'status' => 404 )
			);
		}
	...
	public static function get_widget_schema( $widget_type, $tab = 'content' ) {
		...
		if ( ! $widget ) {
			return new \WP_Error(
				'uael_widget_not_found',
				/* translators: %s: widget type slug */
				sprintf( __( 'Widget type "%s" not found in Elementor.', 'uael' ), sanitize_text_field( $widget_type ) ),
				array( 'status' => 404 )
			);
		}
		...
		$skip_types = array( 'section', 'tabs', 'tab', 'divider', 'raw_html', 'heading', 'deprecated_notice', 'alert', 'notice' );
		...
		foreach ( $controls as $key => $ctrl ) {
			...
			if ( 0 === strpos( $section, 'uael_' ) && false !== strpos( $section, '_promo' ) ) {
				continue;
			}
	...
	private static function map_control_to_schema( $ctrl ) {
		...
		switch ( $type ) {
			...
			case 'border':
				$prop['type']        = 'string';
				$prop['enum']        = array( 'solid', 'double', 'dotted', 'dashed', 'groove', '' );
				$prop['description'] = 'Border type. Sub-keys: {name}_width (dimensions), {name}_color (hex).';
				break;

			case 'background':
				$prop['type']        = 'string';
				$prop['enum']        = array( 'classic', 'gradient', '' );
	...
	public static function get_allowed_widget_types() {
		...
		// UAE (Free / Header Footer Elementor) widgets — only when that plugin
		// is active. In standalone Pro these are not registered in Elementor, so
		// advertising them would offer widgets that cannot actually be inserted.
		if ( defined( 'HFE_VER' ) ) {
			$widgets = array_merge( $widgets, self::get_uael_widget_types() );
		}
	...
	public static function get_pro_alternative( $widget_type ) {
		$uael_widgets = self::get_uael_widget_types();

		foreach ( $uael_widgets as $widget ) {
	 ...
	 */
	public static function get_uael_widget_types_with_pro() {
		return self::get_uael_widget_types();
	}
	 ...
	 */
	private static function get_uael_widget_types() {
	...
	public static function check_widget_requirements( $widget_type ) {
		// Check HFE widgets.
		$uael_slugs = array_column( self::get_uael_widget_types(), 'slug' );
		if ( in_array( $widget_type, $uael_slugs, true ) ) {
			return array(
		...
		if ( 0 === strpos( $widget_type, 'uael-' ) ) {
			return array(
				...
				'is_active'       => class_exists( 'UAEL_Config' ),
			);
		}
