...
class BSF_Rollback_Version {
	 ...
	 */
	public function run() {
		// When the installed version is older than the remote version (second
		// consecutive rollback), BSF's update filter overwrites the rollback
		// package with the latest version's package — skip it while rolling back.
		add_filter( 'bsf_skip_update_transient_data', '__return_true' );
		$this->apply_package();
		$this->upgrade();
		remove_filter( 'bsf_skip_update_transient_data', '__return_true' );
	}
