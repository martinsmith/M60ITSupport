...
if ( ! class_exists( 'BSF_Update_Manager' ) ) {
	...
	class BSF_Update_Manager {
		...
		public function brainstorm_update_plugins_transient( $_transient_data ) {
			...
			return $this->remove_stale_bsf_updates( $_transient_data );
		}
		 ...
		 */
		public function remove_stale_bsf_updates( $_transient_data ) {
			if ( empty( $_transient_data->response ) || ! is_array( $_transient_data->response ) ) {
				return $_transient_data;
			}

			$all_products = self::prepare_plugins_for_update( brainstorm_get_all_products( false, true, false ) );
