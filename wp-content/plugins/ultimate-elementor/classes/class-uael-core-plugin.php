...
class UAEL_Core_Plugin {
	...
	private function setup_actions_filters() {
		...
		// Keep the cached Author Filter list in step with the user and role lifecycle.
		// The list is scoped to the current site, so multisite membership changes
		// count too: granting an existing network user a role here fires
		// add_user_to_blog, not set_user_role.
		$user_cache_hooks = array(
			'user_register',
			'profile_update',
			'deleted_user',
			'set_user_role',
		...
		foreach ( $user_cache_hooks as $user_hook ) {
			add_action( $user_hook, array( 'UltimateElementor\Classes\UAEL_Posts_Helper', 'flush_user_list_cache' ) );
		}
