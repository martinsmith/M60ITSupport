...
class UAEL_Turnstile {
	...
	/**
	 * Get Elementor tablet/mobile breakpoint max-widths.
	 *
	 * @since 1.45.2
	 * @return array Breakpoint max-widths keyed by tablet/mobile.
	 */
	public function get_elementor_breakpoints() {
		$breakpoints = array(
			'tablet' => 1024,
			'mobile' => 767,
		);
		...
		if ( class_exists( '\Elementor\Plugin' ) && isset( \Elementor\Plugin::$instance->breakpoints ) ) {
			...
			if ( isset( $active_breakpoints['tablet'] ) ) {
				$breakpoints['tablet'] = $active_breakpoints['tablet']->get_value();
			}
			if ( isset( $active_breakpoints['mobile'] ) ) {
				$breakpoints['mobile'] = $active_breakpoints['mobile']->get_value();
			}
		}
	 ...
	 *
	 * @since 1.42.2
	 * @since 1.45.2 Added the $size parameter.
	 * @param string       $form_type Type of form.
	 * @param string       $node_id Node ID.
	 * @param string|array $size Turnstile widget size (normal/flexible/compact). Accepts a string or an array
	 *                           with desktop/tablet/mobile keys. Invalid values fall back to the default size.
	 */
	public function render_turnstile_widget( $form_type, $node_id, $size = '' ) {
		$settings  = $this->get_turnstile_settings();
		$widget_id = 'uael-turnstile-' . $form_type . '-' . $node_id;

		$allowed_sizes = array( 'normal', 'flexible', 'compact' );
		$sizes         = is_array( $size ) ? $size : array( 'desktop' => $size );

		$desktop_size = isset( $sizes['desktop'] ) && in_array( $sizes['desktop'], $allowed_sizes, true ) ? $sizes['desktop'] : $settings['size'];
		$tablet_size  = isset( $sizes['tablet'] ) && in_array( $sizes['tablet'], $allowed_sizes, true ) ? $sizes['tablet'] : '';
		$mobile_size  = isset( $sizes['mobile'] ) && in_array( $sizes['mobile'], $allowed_sizes, true ) ? $sizes['mobile'] : '';

		// Elementor breakpoint max-widths used by the JS to resolve the per-device size.
		...
		// phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion -- Cloudflare's loader parses its own script URL and warns on a ver query arg; the CDN manages its own caching.
		wp_enqueue_script(
		...
		<div class="uael-turnstile-container">
			<div class="uael-turnstile-widget">
				<div id="<?php echo esc_attr( $widget_id ); ?>"
					...
					data-size="<?php echo esc_attr( $desktop_size ); ?>"
					<?php echo '' !== $tablet_size ? 'data-size-tablet="' . esc_attr( $tablet_size ) . '"' : ''; ?>
					<?php echo '' !== $mobile_size ? 'data-size-mobile="' . esc_attr( $mobile_size ) . '"' : ''; ?>
					data-bp-tablet="<?php echo esc_attr( $tablet_bp ); ?>"
					data-bp-mobile="<?php echo esc_attr( $mobile_bp ); ?>"
