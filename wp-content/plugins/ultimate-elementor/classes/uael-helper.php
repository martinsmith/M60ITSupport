...
class UAEL_Helper {
	...
	/**
	 * Maximum lifetime of a locally cached reviewer image.
	 *
	 * Clamped so the local copy always stays a short-lived, temporary cache in
	 * line with the Places API policy, regardless of the filtered value.
	 *
	 * @since 1.45.4
	 * @return int Lifetime in seconds ( never more than 30 days ).
	 */
	public static function get_review_image_max_age() {
		/**
		 * Filters the lifetime of a locally cached reviewer image, in seconds.
		 *
		 * The returned value is clamped to a hard maximum of 30 days so the cache
		 * remains temporary per the Places API policy.
		 *
		 * @since 1.45.4
		 * @param int $max_age Lifetime in seconds. Default 30 days.
		 */
		$max_age = (int) apply_filters( 'uael_br_image_cache_lifetime', 30 * DAY_IN_SECONDS );

		return min( 30 * DAY_IN_SECONDS, max( 0, $max_age ) );
	}

	/**
	 * Download a reviewer image and cache it locally.
	 *
	 * Serves reviewer avatars from the site's own uploads directory so that a
	 * visitor's browser never connects to a third-party server (e.g. Google) to
	 * load the image, addressing GDPR concerns for EU sites. Cached copies are
	 * short-lived and refreshed automatically, in line with the Places API
	 * caching policy (Places content must not be stored long-term).
	 *
	 * Intended to run only on the review cache-refresh path.
	 *
	 * @since 1.45.4
	 * @param string $remote_url The remote reviewer image URL from the reviews API.
	 * @return string|false Local image URL on success, false on any failure.
	 */
	public static function localize_review_image( $remote_url ) {

		$remote_url = esc_url_raw( $remote_url );

		// Only handle absolute HTTPS URLs.
		if ( empty( $remote_url ) || 0 !== strpos( $remote_url, 'https://' ) ) {
			return false;
		}

		// Only fetch from known reviewer-image hosts (defense-in-depth against a
		// spoofed or compromised API response pointing at an arbitrary endpoint).
		$host = wp_parse_url( $remote_url, PHP_URL_HOST );

		/**
		 * Filters the host suffixes allowed for local reviewer-image downloads.
		 *
		 * @since 1.45.4
		 * @param array $hosts List of allowed host suffixes.
		 */
		$allowed_hosts = apply_filters( 'uael_br_image_allowed_hosts', array( 'googleusercontent.com', 'ggpht.com' ) );
		...
		$max_age = self::get_review_image_max_age();
		...
		$cache_dir = trailingslashit( $uploads['basedir'] ) . 'uael-business-reviews';
		$cache_url = trailingslashit( $uploads['baseurl'] ) . 'uael-business-reviews';
		$base      = md5( $remote_url );

		// Serve an existing fresh copy without re-downloading (extension unknown).
		// Checked before the failure marker so a fresh, servable file is always
		// used even if an unrelated download recently set the fail transient.
		$existing = glob( trailingslashit( $cache_dir ) . $base . '.*' );

		if ( ! empty( $existing ) ) {
			$file = $existing[0];
			if ( ( time() - filemtime( $file ) ) < $max_age ) {
				return trailingslashit( $cache_url ) . wp_basename( $file );
			}
			wp_delete_file( $file );
		}

		// Skip URLs that recently failed, so a transient error never turns into a
		// repeated blocking download on the refresh path.
		$fail_key = 'uael_br_img_fail_' . md5( $remote_url );

		if ( false !== get_transient( $fail_key ) ) {
			return false;
		}

		// Local writes are only safe with direct filesystem access; on FTP/SSH
		// hosts WP_Filesystem() cannot be initialised unprompted.
		...
		/**
		 * Filters the maximum allowed size, in bytes, of a reviewer image.
		 *
		 * @since 1.45.4
		 * @param int $max_bytes Maximum size in bytes. Default 2 MB.
		 */
		$max_bytes = (int) apply_filters( 'uael_br_image_max_bytes', 2 * MB_IN_BYTES );
		$file_size = (int) filesize( $tmp_file );
		$image     = getimagesize( $tmp_file );

		// Reject oversized responses and anything that is not a valid image.
		if ( $file_size > $max_bytes || false === $image ) {
		...
		if ( ! $wp_filesystem || ! $wp_filesystem->move( $tmp_file, $filepath, true ) ) {
			wp_delete_file( $tmp_file );
			set_transient( $fail_key, 1, HOUR_IN_SECONDS );
			return false;
		}

		// Drop a blank index.php once to suppress directory listing.
		...
		return trailingslashit( $cache_url ) . $filename;
	}

	/**
	 * Delete locally cached reviewer images older than their allowed lifetime.
	 *
	 * A lightweight garbage-collection sweep, run on the review cache-refresh
	 * path, that removes orphaned files left behind when providers rotate image
	 * URLs. Keeps the cache directory from growing unbounded.
	 *
	 * @since 1.45.4
	 * @return void
	 */
	public static function gc_review_image_cache() {
	...
	public static function get_api_authentication( $source, $api_key = '' ) {
		...
		// Clear locally cached reviewer images so they are re-fetched with the reviews.
		self::purge_review_image_cache();
	}

	/**
	 * Delete the locally cached reviewer images directory.
	 *
	 * Called when the reviews cache is cleared so local images stay in sync with
	 * the review data and never persist longer than the reviews they belong to.
	 *
	 * @since 1.45.4
	 * @return void
	 */
	public static function purge_review_image_cache() {
