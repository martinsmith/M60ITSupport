...
class UAEL_Angie {
	 ...
	 * @var string
	 */
	const REST_NAMESPACE = 'uael/v1';

	/**
	 * Registry instance.
	 *
	 * @var UAEL_Ability_Registry
	 */
	private $registry;
	 ...
	 * Constructor.
	 *
	 * @param UAEL_Ability_Registry $registry The ability registry.
	 */
	public function __construct( UAEL_Ability_Registry $registry ) {
	 ...
	 * - POST /uael/v1/angie/abilities/ → returns all ability schemas
	 * - POST /uael/v1/angie/{ability_id}/ → executes a specific ability
	 *
	 * @return void
	 */
	 ...
	 * @var array
	 */
	private static $angie_hidden_abilities = array(
	 ...
	 *
	 * Only enqueues when Angie is active (ANGIE_VERSION defined) and both the
	 * abilities feature and the Angie integration are explicitly enabled in
	 * settings. Fails closed when either flag is unset.
	 *
	 * @return void
	 */
	public function enqueue_scripts() {
		...
		wp_enqueue_script_module(
			'uael-angie-mcp-server',
			UAEL_URL . 'angie/uael-angie-mcp-server.min.js',
			array(),
			defined( 'UAEL_VER' ) ? UAEL_VER : '1.0.0',
	...
	public static function ability_name_to_angie_id( $ability_name ) {
		...
		$prefix = UAEL_Ability_Registry::PREFIX;
	...
	private function angie_id_to_handler_name( $angie_id ) {
		...
		foreach ( $this->registry->get_handler_names() as $name ) {
			if ( self::ability_name_to_angie_id( UAEL_Ability_Registry::PREFIX . $name ) === $angie_id ) {
				return $name;
			}
		}
		...
		if ( ! $this->registry->is_registered( $handler_name ) ) {
			...
			if ( 'allow_modifications' === $reason ) {
				$message = sprintf(
					...
					__( 'BLOCKED: Modifications are disabled by the site administrator. Do NOT attempt to enable this yourself. Tell the user: "Allow Modifications" must be turned on in UAE Settings → AI Tools at %s', 'uael' ),
					$settings_url
				);
			} else {
				$message = sprintf(
					/* translators: 1: ability ID, 2: settings URL */
					__( 'BLOCKED: The ability "%1$s" has been disabled by the site administrator. Do NOT retry. Tell the user to enable it in UAE Settings → AI Tools at %2$s', 'uael' ),
					$angie_id,
					$settings_url
				);
			}
	...
	private function verify_rest_nonce( $request ) {
		...
		if ( ! wp_verify_nonce( $nonce, 'wp_rest' ) ) {
			return new \WP_Error(
				'uael_rest_invalid_nonce',
				__( 'Invalid or missing nonce.', 'uael' ),
				array( 'status' => 403 )
			);
		}
