...
<g>
	...
	public function get_google_api( $settings ) {
		...
		$localize_images = $this->should_localize_reviewer_images( $settings );

		// Vary the cache key by the localize setting: the cached payload now holds
		// either local or remote image URLs, so widgets in different modes must not
		// share (and overwrite) one entry, which would silently leak Google URLs.
		$transient_name = 'uael_reviews_' . $placeid . ( $localize_images ? '_local' : '' );
		...
		if ( false === $result || ( is_user_logged_in() && current_user_can( 'manage_options' ) ) ) {
			...
			// Download reviewer images to the local server (GDPR) before caching,
			// so subsequent renders read local URLs and never hit Google.
			if ( $localize_images ) {
				$this->localize_google_result_images( $result );
			}
			...
			// In local mode, cap the review cache to the image lifetime so the
			// local avatar copies are refreshed within 30 days regardless of the
			// chosen refresh interval - keeping them a temporary Places-policy cache.
			if ( $localize_images ) {
				$expire_time = min( $expire_time, UAEL_Helper::get_review_image_max_age() );
			}
	...
	/**
	 * Whether Google reviewer images should be cached and served locally.
	 *
	 * Used both to vary the reviews cache key and to gate the download, so the
	 * key and the cached payload can never disagree about which mode is active.
	 *
	 * @since 1.45.4
	 * @param array $settings The widget settings array.
	 * @return bool True when reviewer images should be stored locally.
	 */
	public function should_localize_reviewer_images( $settings ) {

		$localize_images = ( 'yes' === $this->get_instance_value( 'reviewer_image' ) && 'local' === $this->get_instance_value( 'reviewer_image_source' ) );

		/**
		 * Filters whether Google reviewer images are downloaded and served locally.
		 *
		 * Allows forcing GDPR-friendly local images site-wide in code.
		 *
		 * @since 1.45.4
		 * @param bool   $localize_images Whether to localize reviewer images.
		 * @param string $type            The reviews source. Always 'google' here.
		 * @param array  $settings        The widget settings array.
		 */
		return (bool) apply_filters( 'uael_br_localize_reviewer_images', $localize_images, 'google', $settings );
	}

	/**
	 * Download Google reviewer images to the local server.
	 *
	 * Rewrites the reviewer image URLs inside the decoded API payload to locally
	 * cached copies before the payload is stored in the transient, so the review
	 * data is served entirely from the site's own server (GDPR). Runs only on the
	 * cache-refresh path, not on every render. Yelp is intentionally excluded as
	 * its terms restrict re-serving assets. Callers must gate this on
	 * should_localize_reviewer_images().
	 *
	 * @since 1.45.4
	 * @param object $result The decoded Google Places API response. Mutated in place.
	 * @return void
	 */
	public function localize_google_result_images( $result ) {

		if ( ! is_object( $result ) ) {
			return;
		}

		// Remove expired local copies so the cache never outlives its usefulness.
		UAEL_Helper::gc_review_image_cache();

		if ( 'yes-new' === self::$google_api_status ) {
			// phpcs:disable WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase -- Property names come from the external API.
			if ( isset( $result->reviews ) && is_array( $result->reviews ) ) {
				foreach ( $result->reviews as $review ) {
					if ( ! empty( $review->authorAttribution->photoUri ) ) {
						$local_image                         = UAEL_Helper::localize_review_image( $review->authorAttribution->photoUri );
						$review->authorAttribution->photoUri = ( false !== $local_image ) ? $local_image : '';
					}
				}
			}
			// phpcs:enable
		} elseif ( isset( $result->result->reviews ) && is_array( $result->result->reviews ) ) {
			foreach ( $result->result->reviews as $review ) {
				if ( ! empty( $review->profile_photo_url ) ) {
					$local_image               = UAEL_Helper::localize_review_image( $review->profile_photo_url );
					$review->profile_photo_url = ( false !== $local_image ) ? $local_image : '';
				}
			}
		}
	}
