...
class Timeline extends Common_Widget {
	...
	public function register_content_query_controls() {
		...
		// Options lists below are only consulted when the editor panel draws a
		// control, but Elementor registers the whole stack on the frontend too.
		// Resolve the context once and skip those queries everywhere else.
		$in_editor = UAEL_Posts_Helper::is_editor_context();
		...
		foreach ( $post_types as $key => $type ) {
			...
			if ( ! empty( $taxonomy ) ) {
				...
				foreach ( $taxonomy as $index => $tax ) {
					...
					// Only query terms in editor contexts — the options list is never consulted on the frontend.
					if ( $in_editor ) {
					...
					// Editor: register only when terms exist (avoid empty dropdowns).
					// Frontend: always register so Elementor can resolve control defaults (e.g. filter_rule = 'IN').
					if ( ! $in_editor || ! empty( $related_tax ) ) {
						$this->add_control(
				...
				$this->add_control(
					...
					array(
						...
						'default'     => '',
						'label_block' => true,
						// The saved value is passed straight to WP_Query and is never
						// validated against this list, so an empty options array on the
						// frontend cannot change what renders.
						'options'     => $in_editor ? UAEL_Posts_Helper::get_users() : array(),
						'condition'   => array(
							'query_type'    => 'custom',
							'timeline_type' => 'posts',
						),
