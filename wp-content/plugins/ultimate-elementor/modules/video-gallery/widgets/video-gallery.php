...
class Video_Gallery extends Common_Widget {
			...
			jQuery( document ).ready( function( $ ) {
				...
				$( '.uael-video-gallery-wrap' ).each( function() {
					...
					if ( ! selector.hasClass( 'uael-video-gallery-filter' ) ) {
						...
						/**
						 * Non-filterable grid layout.
						 *
						 * The video thumbnails derive their height from a percentage
						 * `padding-bottom` aspect ratio, which contributes no intrinsic
						 * height. Inside an Elementor flex container the parent cannot
						 * resolve the wrapper's height and the section collapses, causing
						 * the gallery to overflow onto the following content (mobile only,
						 * where the stacked rows exceed the viewport). Locking an explicit
						 * min-height — recomputed on resize and when the wrapper becomes
						 * visible — gives the wrapper a definite height and resolves the
						 * collapse. The filterable layout below already avoids this via
						 * Isotope's explicit sizing.
						 */
						if ( selector.hasClass( 'uael-vg__layout-grid' ) ) {

							var uaelVgResizeTimer = null;

							/**
							 * Lock the min-height to the natural content height. Skipped
							 * while the wrapper is hidden (e.g. an inactive Tabs, Nested
							 * Tabs, Accordion or Toggle panel), where scrollHeight resolves
							 * to 0; the ResizeObserver below recomputes once it is revealed.
							 * Re-applying the same value leaves the box size unchanged, so
							 * the observer settles once the height stops changing.
							 */
							var uaelVgSyncHeight = function() {
								if ( ! selector.is( ':visible' ) ) {
									return;
								}
								selector.css( 'min-height', '' );
								selector.css( 'min-height', selector[0].scrollHeight + 'px' );
							};

							selector.imagesLoaded( uaelVgSyncHeight );
							uaelVgSyncHeight();

							$( window ).off( 'resize.uaelVgGrid' + $node_id + ' orientationchange.uaelVgGrid' + $node_id );
							$( window ).on( 'resize.uaelVgGrid' + $node_id + ' orientationchange.uaelVgGrid' + $node_id, function() {
								clearTimeout( uaelVgResizeTimer );
								uaelVgResizeTimer = setTimeout( uaelVgSyncHeight, 150 );
							} );

							/**
							 * Recompute whenever the wrapper's rendered size changes. Inside
							 * a Tabs/Accordion/Toggle panel the wrapper is `display:none` on
							 * load, so the initial sync runs while hidden and does nothing.
							 * Activating the panel fires no window resize, but it does resize
							 * the wrapper from 0 to its rendered size, which the observer
							 * catches — giving a container-agnostic recompute on reveal.
							 * IntersectionObserver is unsuitable here as it does not notify
							 * for elements revealed out of a `display:none` subtree.
							 */
							if ( 'undefined' !== typeof window.ResizeObserver ) {

								if ( selector[0].uaelVgObserver ) {
									selector[0].uaelVgObserver.disconnect();
								}

								var uaelVgObserver = new ResizeObserver( function() {
									uaelVgSyncHeight();
								} );

								selector[0].uaelVgObserver = uaelVgObserver;
								uaelVgObserver.observe( selector[0] );
							}
						}
	...
	public function get_play_button() {
		...
		if ( 'icon' === $settings['play_source'] ) {
			...
			<?php
			if ( UAEL_Helper::is_elementor_updated() ) {
				if ( ! empty( $settings['play_icon'] ) || ! empty( $settings['new_play_icon']['value'] ) ) :
