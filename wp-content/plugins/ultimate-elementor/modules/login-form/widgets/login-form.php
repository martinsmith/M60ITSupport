	...
	protected function register_general_controls() {
		...
		// Turnstile Security Control.
			...
			$this->add_responsive_control(
				'turnstile_size',
				array(
					'label'       => __( 'Turnstile Size', 'uael' ),
					'type'        => Controls_Manager::SELECT,
					'default'     => 'normal',
					'options'     => array(
						'normal'   => __( 'Normal', 'uael' ),
						'flexible' => __( 'Flexible (Full Width)', 'uael' ),
						'compact'  => __( 'Compact', 'uael' ),
					),
					'description' => __( 'Flexible automatically matches the width of the form container.', 'uael' ),
					'condition'   => array(
			...
			$this->add_responsive_control(
				'turnstile_align',
				array(
					'label'                => __( 'Turnstile Alignment', 'uael' ),
					...
					'selectors'            => array(
						'{{WRAPPER}} .uael-turnstile-widget' => 'display: flex; {{VALUE}}',
						'{{WRAPPER}} .uael-turnstile-widget .cf-turnstile' => 'margin-left: 0; margin-right: 0;',
					),
					'condition'            => array(
						'enable_turnstile' => 'yes',
			...
			$this->add_responsive_control(
				'turnstile_spacing',
				array(
					'label'      => __( 'Turnstile Bottom Spacing', 'uael' ),
					'type'       => Controls_Manager::SLIDER,
					'size_units' => array( 'px' ),
					'range'      => array(
						'px' => array(
					...
					'selectors'  => array(
						'{{WRAPPER}} .uael-turnstile-container' => 'margin-bottom: {{SIZE}}{{UNIT}};',
						'{{WRAPPER}} .uael-turnstile-widget' => 'margin-bottom: 0;',
					),
					'condition'  => array(
						'enable_turnstile' => 'yes',
					),
							...
							if ( ! empty( $settings['enable_turnstile'] ) && 'yes' === $settings['enable_turnstile'] ) {
								...
								if ( $turnstile->is_login_turnstile_enabled() ) {
									$turnstile_size = array(
										'desktop' => isset( $settings['turnstile_size'] ) ? $settings['turnstile_size'] : 'normal',
										'tablet'  => isset( $settings['turnstile_size_tablet'] ) ? $settings['turnstile_size_tablet'] : '',
										'mobile'  => isset( $settings['turnstile_size_mobile'] ) ? $settings['turnstile_size_mobile'] : '',
									);
									$turnstile->render_turnstile_widget( 'login', $node_id, $turnstile_size );
								}
							}
							?>
				...
				<div class="uael-login-form uael-form">
					<div class="elementor-form-fields-wrapper">
						...
						<div class="elementor-field-group elementor-column elementor-field-type-submit elementor-col-100">
							<?php
							$editor_turnstile             = \UltimateElementor\Classes\UAEL_Turnstile::get_instance();
							$editor_turnstile_settings    = $editor_turnstile->get_turnstile_settings();
							$editor_turnstile_breakpoints = $editor_turnstile->get_elementor_breakpoints();
							if ( ! empty( $editor_turnstile_settings['site_key'] ) && ! empty( $editor_turnstile_settings['secret_key'] ) ) :
								?>
								<# if ( 'yes' === settings.enable_turnstile ) { #>
									<div class="uael-turnstile-container">
										<div class="uael-turnstile-widget">
											<div class="cf-turnstile"
												data-sitekey="<?php echo esc_attr( $editor_turnstile_settings['site_key'] ); ?>"
												data-theme="<?php echo esc_attr( $editor_turnstile_settings['theme'] ); ?>"
												data-size="{{ settings.turnstile_size || 'normal' }}"
												<# if ( settings.turnstile_size_tablet ) { #>data-size-tablet="{{ settings.turnstile_size_tablet }}"<# } #>
												<# if ( settings.turnstile_size_mobile ) { #>data-size-mobile="{{ settings.turnstile_size_mobile }}"<# } #>
												data-bp-tablet="<?php echo esc_attr( $editor_turnstile_breakpoints['tablet'] ); ?>"
												data-bp-mobile="<?php echo esc_attr( $editor_turnstile_breakpoints['mobile'] ); ?>"
												data-action="uael_login_form">
											</div>
										</div>
									</div>
								<# } #>
