	...
	protected function render() {
		...
		<div class="uael-howto-wrapper">
			...
			<script type="application/ld+json">
				...
				"@type": "HowTo",
				"name": "<?php echo ! empty( $settings['title'] ) ? esc_html( $settings['title'] ) : ''; ?>",
				"description": "<?php echo ! empty( $settings['description'] ) ? esc_html( $settings['description'] ) : ''; ?>",
				<?php if ( ! empty( $settings['image']['url'] ) ) { ?>
					"image": {
						"@type": "ImageObject",
						"url": "<?php echo ! empty( $settings['image']['url'] ) ? esc_url( $settings['image']['url'] ) : ''; ?>",
						"height": "406",
						"width": "305"
					},
				<?php } ?>
				<?php if ( ! empty( $settings['estimated_cost'] ) && 'yes' === $settings['show_cost'] ) { ?>
					"estimatedCost": {
						"@type": "MonetaryAmount",
						"currency": "<?php echo ! empty( $settings['cost_iso_code'] ) ? esc_html( $settings['cost_iso_code'] ) : ''; ?>",
						"value": "<?php echo ! empty( $settings['estimated_cost'] ) ? esc_html( $settings['estimated_cost'] ) : ''; ?>"
					},
				<?php } ?>
				...
				<?php if ( ! empty( $calculate_total_time ) && 'yes' === $settings['show_time'] ) { ?>
					...
					<?php if ( isset( $calculate_total_time ) ) { ?>
						"PT<?php echo esc_html( $calculate_total_time ); ?>M"
					<?php } ?>
				<?php } ?>
