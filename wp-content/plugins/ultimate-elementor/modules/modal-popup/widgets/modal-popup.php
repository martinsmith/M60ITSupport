...
class Modal_Popup extends Common_Widget {
		...
		switch ( $content_type ) {
			...
			case 'photo':
				if ( isset( $dynamic_settings['ct_photo']['url'] ) ) {
					...
					$output_html = '<img src="' . esc_url( $dynamic_settings['ct_photo']['url'] ) . '" alt="' . esc_attr( Control_Media::get_image_alt( $dynamic_settings['ct_photo'] ) ) . '" />';
				} else {
			...
			case 'iframe':
				if ( 'yes' === $dynamic_settings['async_iframe'] ) {
					...
					$output_html = '<div class="uael-modal-content-type-iframe" data-src="' . esc_url( $dynamic_settings['iframe_url'] ) . '" frameborder="0" allowfullscreen></div>';
				} else {
					$output_html = '<iframe src="' . esc_url( $dynamic_settings['iframe_url'] ) . '" class="uael-content-iframe" frameborder="0" width="100%" height="100%" allowfullscreen></iframe>';
				}
				break;
			case 'saved_rows':
	...
	public function get_video_embed( $settings, $node_id ) {
		...
		if ( 'youtube' === $settings['content_type'] ) {
			...
			$html .= '<div class="uael-modal-iframe uael-video-player" data-src="youtube" data-id="' . esc_attr( $vid_id ) . '" data-thumb="' . esc_attr( $thumb ) . '" data-sourcelink="' . esc_attr( 'https://www.youtube.com/embed/' . $vid_id . $video_data ) . '" data-play-icon="' . esc_attr( $play_icon ) . '"></div>';
			...
			if ( '' !== $vid_id && 0 !== $vid_id ) {
				$html .= '<div class="uael-modal-iframe uael-video-player" data-src="vimeo" data-id="' . esc_attr( $vid_id ) . '" data-thumb="' . esc_attr( $thumb ) . '" data-sourcelink="' . esc_attr( 'https://player.vimeo.com/video/' . $vid_id . $video_data ) . '" data-play-icon="' . esc_attr( $play_icon ) . '" ></div>';
			}
		}
		$html .= '</div>';
		return $html;
	...
	protected function render_button_text() {
		...
		<span <?php echo wp_kses_post( $this->get_render_attribute_string( 'content-wrapper' ) ); ?>>
			...
			if ( UAEL_Helper::is_elementor_updated() ) {
				...
				?>
				<?php if ( ! empty( $settings['btn_icon'] ) || ! empty( $settings['new_btn_icon']['value'] ) ) : ?>
	...
	protected function render_close_icon() {
		...
		if ( 'icon' === $settings['close_source'] ) {
			...
			?>
			<img class="uael-close-image" src="<?php echo ( isset( $settings['close_photo']['url'] ) ) ? esc_url( $settings['close_photo']['url'] ) : ''; ?>" alt="<?php echo ( isset( $settings['close_photo']['url'] ) ) ? esc_attr( Control_Media::get_image_alt( $settings['close_photo'] ) ) : ''; ?>"/>
			<?php
		}
		?>
		</span>
		...
		if ( 'button' === $settings['modal_on'] ) {
			...
			switch ( $settings['modal_on'] ) {
				case 'text':
					...
					$inner_html = wp_kses_post( $settings['modal_text'] );
				...
				case 'icon':
					...
					if ( UAEL_Helper::is_elementor_updated() ) {
						...
						if ( $icon_is_new || $icon_migrated ) {
							...
							$inner_html = '<i class="' . esc_attr( $settings['icon'] ) . '" aria-hidden="true"></i>';
						}
					} elseif ( ! empty( $settings['icon'] ) ) {
						$inner_html = '<i class="' . esc_attr( $settings['icon'] ) . '" aria-hidden="true"></i>';
					}
				...
				case 'photo':
					...
					$inner_html = '<img class="uael-modal-photo" src="' . esc_url( $url ) . '" alt="' . esc_attr( Control_Media::get_image_alt( $settings['photo'] ) ) . '">';
