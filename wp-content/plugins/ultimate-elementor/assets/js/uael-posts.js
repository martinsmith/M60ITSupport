( function( $ ) {
	...
	/**
	 * Returns the per-widget pagination state.
	 *
	 * Each Posts widget instance keeps its own counter, load flag, total and
	 * loader reference. These were previously declared at module scope and
	 * shared across every widget instance on the page, which caused multiple
	 * infinite-scroll Posts widgets to interfere with each other's pagination.
	 *
	 * @since 1.45.2
	 * @param {Object} $scope jQuery widget scope element.
	 * @return {Object} Per-widget state object.
	 */
	var getState = function( $scope ) {
		var state = $scope.data( 'uaelPostsState' );
		...
		if ( ! state ) {
			...
			$scope.data( 'uaelPostsState', state );
		}
		...
		if ( selector.hasClass( 'uael-post-infinite-scroll' ) && selector.hasClass( 'uael-post-infinite__event-scroll' ) ) {
			...
			$( window ).on( 'scroll', function () {
				...
				if( ( $( window ).scrollTop() + windowHeight50 ) >= ( $scope.find( '.uael-post-wrapper:last' ).offset().top ) ) {
					...
					state.total = $scope.find( '.uael-post__footer .uael-grid-pagination' ).data( 'total' );

					if( true == state.loadStatus ) {
