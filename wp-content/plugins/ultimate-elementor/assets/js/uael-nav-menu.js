( function( $ ) {
	 ...
	 */
	function uael_sanitize_icon_html( markup ) {
	...
	function _toggleClick( id ){
		...
		nav_toggle.off( 'click keyup' ).on( 'click keyup', function( event ) {
			...
			if ( $this.hasClass( 'uael-active-menu' ) ) {
				...
				// hamburger icon. See uael_sanitize_icon_html above.
				if ( 'string' === typeof toggle_icon ) {
					$element.find( '.uael-nav-menu-icon' ).html( uael_sanitize_icon_html( toggle_icon ) );
				}
				...
				if ( 'string' === typeof close_icon ) {
					$element.find( '.uael-nav-menu-icon' ).html( uael_sanitize_icon_html( close_icon ) );
				}
