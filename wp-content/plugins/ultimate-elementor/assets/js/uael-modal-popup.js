( function( $ ) {
	...
	$( document ).on( 'uael_modal_popup_init', function( e, node_id ) {
		...
		if ( 'youtube' == content_type || 'vimeo' == content_type ) {
			...
			if( 0 == overlay_node.find( '.uael-video-player iframe' ).length ) {

				overlay_node.find( '.uael-video-player' ).each( function( index, value ) {
					...
					var div = $( "<div/>" ),
						...
						// Build the thumbnail markup with DOM APIs so the data-* values are
						// assigned as attribute/class strings and never parsed as live HTML.
						var thumb = $( '<img/>', { src: $this.data( 'thumb' ) } );
						var play  = $( '<div/>' ).addClass( 'play' ).addClass( String( $this.data( 'play-icon' ) ) );
						div.empty().append( thumb ).append( play );
